...
class FavoriteListScreen extends StatefulWidget {
  static String id = Routes.favorite;
  final String title;

  const FavoriteListScreen({Key? key, required this.title}) : super(key: key);
                    ...
                    child: SingleChildScrollView(
                      child: Padding(
                        ...
                        child: OverlayView(
                          item_1: Column(
                            children: <Widget>[
                              if (favoriteProvider.favorites.length > 0) ...[
                                for (int index = 0; index < postProvider.posts.length; index++) if (favoriteProvider.favorites.map((favorite) => favorite['post']).toList().contains(postProvider.posts[index]['id']) == true) GestureDetector(
                                  onTap: () {
                                    postProvider.post = postProvider.posts.where((post) => post['id'] == postProvider.posts[index]['id']).toList()[0];
                                    Get.toNamed(Routes.post_retrieve, arguments: {'post': postProvider.post['id']},);
                                  },
                                  child: PostFavorite(item: new Post(postProvider.posts[index]['id'], postProvider.posts[index]['title'], postProvider.posts[index]['content'], postProvider.posts[index]['phone'], postProvider.posts[index]['service'], postProvider.posts[index]['condition'], postProvider.posts[index]['location'], num.tryParse(postProvider.posts[index]['price'].toString())?.toDouble(), postProvider.posts[index]['currency'], postProvider.posts[index]['promotion'], postProvider.posts[index]['photos'], postProvider.posts[index]['view'], postProvider.posts[index]['reviews']), functionCallback: (favorite) {
                                    if (favorite != null) {
